...
def get_64tass_operands(oper):
    if oper.endswith(g_and_255):
        ...
        for suffix in g_tolower_suffixes:
            if oper.endswith(suffix):
                return oper[:-len(suffix)]+suffix.lower()
...
def fixup2(options,fout):
    ...
    for output_line in output_lines: print(output_line,file=fout)
